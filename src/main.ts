...
function initialiseGame(app: Application) {
  ...
  app.ticker.add(({ deltaTime }) => {
    ...
    if (isPointerDown || isTouch) {
      ...
      shipVelocity.y += getYVector(ship.rotation, MAX_SPEED) * deltaTime;
      shipVelocity.x += -getXVector(ship.rotation, MAX_SPEED * deltaTime);
      ...
      if (isAcceleratorPressed) {
        ...
        shipVelocity.y += getYVector(ship.rotation, MAX_SPEED) * speedFactor * deltaTime;
        shipVelocity.x += -getXVector(ship.rotation, MAX_SPEED) * speedFactor * deltaTime;
      }
      else {
      ...
      if (isBrakePressed) {
        ...
        const xVelocity = -getXVector(velocityAngle, MAX_SPEED) * deltaTime;
        const yVelocity = getYVector(velocityAngle, MAX_SPEED) * deltaTime;
        ...
        if (shipVelocity.x !== 0) {
          shipVelocity.x += xVelocity * deltaTime;
        }
        if (shipVelocity.y !== 0) {
          shipVelocity.y -= yVelocity * deltaTime;
        }
      }
    }
